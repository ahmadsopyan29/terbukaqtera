...
extract($_POST);
if ($level=='0' || $level=='1') {
  $selectabsen = mysqli_query($mysqli,"SELECT * FROM absen WHERE nik='$nik' GROUP BY tanggal DESC");
  ...
  $dia=$_SESSION['nik'];
  $selectabsen = mysqli_query($mysqli,"SELECT * FROM absen WHERE nik='$dia' GROUP BY tanggal DESC");
    ...
    <table id="example" class="table table-bordered table-hover" role="grid" aria-describedby="tabelpegawai_info">
      ...
      if(! empty($tanggal)){
        while ($dataabsen = mysqli_fetch_array($selectabsen)) {
          ...
          $data2=$dataabsen['nik'];
          $masuk=mysqli_query($mysqli,"SELECT * FROM absen WHERE tanggal='$data1' && nik='$data2' ORDER BY jam asc LIMIT 1");
          $msk=mysqli_fetch_assoc($masuk);
          $idmsk = $msk['idabsen'];
          $keluar=mysqli_query($mysqli,"SELECT * FROM absen WHERE tanggal='$data1' && nik='$data2' && status = 1 ORDER BY jam desc LIMIT 1");
         ...
         // $sanksi=$telat->format('%I') * 500;
          ...
          if ( $status1 == 1 ){
            echo  "<td><a href='tampil_map.php?id=".$idmsk."'>Lihat Lokasi Dan Foto</a></td>";
          }else{
            echo "<td>-</td>";
          }
          ...
          if ( $status2 == 1 ){
            echo  "<td><a href='tampil_map.php?id=".$idklr."'>Lihat Lokasi Dan Foto</a></td>";
          }else{
            echo "<td>-</td>";
          }
          ...
          if ($level=='0' || $level=='1') {
            echo "<td><a href='formabsen.php?aksi=ubah&id=".$idabsen."'>Ubah</a> |
                  <a href='dataabsen_all.php?tanggal=$data1&&nik=$nik'>Show All</a></td>";
                }
